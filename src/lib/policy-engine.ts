...
/* ================================================================
   PART 1027: PHYSICAL METAL RISK MODIFIER
   ================================================================
   FinCEN 31 CFR Part 1027 — Dealers in Precious Metals.
   Evaluates physical supply chain risk and IRS Form 8300
   applicability for cash / cash-equivalent settlements.

   This module is additive — existing checkBlockers /
   runComplianceChecks signatures are NOT modified. Callers
   compose results via helper functions.
   ================================================================ */
...
/**
 * Rails that qualify as "cash or cash equivalents" under
 * IRS/FinCEN Form 8300 (26 U.S.C. § 6050I).
 * Cashier's checks, bank drafts, traveler's checks, and money orders
 * with face values ≤ $10,000 are treated as cash. We flag them
 * conservatively — compliance clears the blocker after review.
 */
export const CASH_EQUIVALENT_RAILS: ReadonlySet<SettlementRail> = new Set<SettlementRail>([
  "CASH",
  "CASHIERS_CHECK",
  "MONEY_ORDER",
  "BANK_DRAFT",
...
export interface PhysicalMetalContext {
  /** Classification of the physical asset */
  assetClass: AssetClassification;
  /** How the metal leaves (or stays in) the vault */
  deliveryMethod: DeliveryMethod;
  /** Funding rail used for settlement */
  settlementRail: SettlementRail;
  /** Total settlement amount in USD */
  amountUsd: number;
}

export interface PhysicalMetalRiskResult {
  /** Product risk weight multiplier (1.0 = standard, 3.0 = high-risk) */
  productRiskWeight: number;
  /** TRI adjustment from physical delivery method (+2 for loadout, 0 otherwise) */
  deliveryEscalation: number;
  /** Net TRI adjustment to apply via applyPhysicalMetalTRIAdjustment */
  triAdjustment: number;
  /** Whether the asset class forces manual compliance review */
  requiresManualReview: boolean;
  /** Whether IRS/FinCEN Form 8300 filing is triggered */
  form8300Required: boolean;
  /** BLOCK/WARN/INFO blockers generated by this evaluation */
  blockers: PolicyBlocker[];
  /** Compliance checks generated by this evaluation */
  complianceChecks: ComplianceCheck[];
}

/* ── Risk Weight Constants ── */

/** Standard risk weight for LBMA Good Delivery bars */
const PRODUCT_RISK_STANDARD = 1.0;

/**
 * High-risk multiplier for Scrap, Doré, or Unknown classification.
 * Forces manual compliance review before settlement can proceed.
 */
const PRODUCT_RISK_HIGH = 3.0;

/** TRI band escalation for physical vault loadout */
const PHYSICAL_LOADOUT_TRI_ESCALATION = 2;

/** Form 8300 threshold in USD */
const FORM_8300_THRESHOLD_USD = 10_000;

/* ── Core Evaluation ── */

/**
 * Evaluate physical metal risks under Part 1027.
 *
 * Returns blockers, compliance checks, and TRI adjustments
 * that callers merge into the main pipeline results.
 */
export function evaluatePhysicalMetalRisk(
  ctx: PhysicalMetalContext,
): PhysicalMetalRiskResult {
  const blockers: PolicyBlocker[] = [];
  const complianceChecks: ComplianceCheck[] = [];

  /* ── Rule 1: Product Risk Classification ── */
  let productRiskWeight = PRODUCT_RISK_STANDARD;
  let requiresManualReview = false;

  if (ctx.assetClass === "LBMA_GOOD_DELIVERY") {
    complianceChecks.push({
      id: "pm-asset",
      name: "Physical Metal Classification",
      result: "PASS",
      detail: "Asset classified as LBMA Good Delivery — standard risk weight (1.0×).",
    });
  } else {
    productRiskWeight = PRODUCT_RISK_HIGH;
    requiresManualReview = true;

    const label = ctx.assetClass === "UNREFINED_DORE"
      ? "Unrefined Doré"
      : ctx.assetClass === "SCRAP"
      ? "Scrap Metal"
    ...
    blockers.push({
      ...
      detail: `Asset classified as ${label} — high-risk weight (${PRODUCT_RISK_HIGH}×). Settlement locked pending manual compliance review per Part 1027.`,
    });
    ...
    complianceChecks.push({
      ...
      name: "Physical Metal Classification",
      result: "FAIL",
      detail: `${label} — high-risk multiplier (${PRODUCT_RISK_HIGH}×). Manual review required.`,
    });
  }

  /* ── Rule 2: Physical Delivery Escalation ── */
  let deliveryEscalation = 0;

  if (ctx.deliveryMethod === "PHYSICAL_LOADOUT") {
    deliveryEscalation = PHYSICAL_LOADOUT_TRI_ESCALATION;

    blockers.push({
      id: "pm-loadout",
      severity: "WARN",
      title: "Physical Vault Loadout",
      detail: `Physical withdrawal of metal from vault. TRI escalated by +${PHYSICAL_LOADOUT_TRI_ESCALATION} bands per Part 1027 physical delivery policy.`,
    });

    complianceChecks.push({
      id: "pm-delivery",
      name: "Delivery Method Risk",
      result: "WARN",
      detail: `Physical loadout — TRI escalated +${PHYSICAL_LOADOUT_TRI_ESCALATION}. Enhanced chain-of-custody documentation required.`,
    });
  } else {
    complianceChecks.push({
      id: "pm-delivery",
      name: "Delivery Method Risk",
      result: "PASS",
      detail: ctx.deliveryMethod === "VAULT_TO_VAULT"
        ? "Vault-to-vault transfer — standard risk."
        : "Digital title transfer — no physical delivery risk.",
    });
  }

  /* ── Rule 3: Form 8300 Detection ── */
  let form8300Required = false;
  const isCashEquivalent = CASH_EQUIVALENT_RAILS.has(ctx.settlementRail);

  if (isCashEquivalent && ctx.amountUsd > FORM_8300_THRESHOLD_USD) {
    form8300Required = true;

    blockers.push({
      id: "form-8300-required",
      severity: "BLOCK",
      title: "FORM 8300 REQUIRED",
      detail: `Settlement of $${ctx.amountUsd.toLocaleString("en-US", { minimumFractionDigits: 2 })} via ${ctx.settlementRail.replace(/_/g, " ")} exceeds $10,000 threshold. IRS/FinCEN Form 8300 must be filed within 15 days. Settlement status LOCKED until compliance clears.`,
    });

    complianceChecks.push({
      id: "pm-form8300",
      name: "Form 8300 Applicability",
      result: "FAIL",
      detail: `Cash-equivalent rail (${ctx.settlementRail}) with amount $${ctx.amountUsd.toLocaleString("en-US")} > $10,000 — Form 8300 filing mandatory.`,
    });
  } else if (isCashEquivalent) {
    complianceChecks.push({
      id: "pm-form8300",
      name: "Form 8300 Applicability",
      result: "PASS",
      detail: `Cash-equivalent rail but amount $${ctx.amountUsd.toLocaleString("en-US")} ≤ $10,000 — Form 8300 not triggered. Monitor for aggregation.`,
    });
  } else {
    complianceChecks.push({
      id: "pm-form8300",
      name: "Form 8300 Applicability",
      result: "PASS",
      detail: `Non-cash settlement rail (${ctx.settlementRail}) — Form 8300 not applicable.`,
    });
  }

  /* ── Composite TRI adjustment ── */
  const triAdjustment = deliveryEscalation;
...
/**
 * Apply physical metal TRI adjustment to a base TRI result.
 * Clamps final score to [1, 10] and recalculates the band.
 */
export function applyPhysicalMetalTRIAdjustment(
  baseTRI: TRIResult,
  adjustment: number,
): TRIResult {
  if (adjustment === 0) return baseTRI;

  const adjusted = Math.min(10, Math.max(1, baseTRI.score + adjustment));
  const band = adjusted <= 3 ? "green" as const : adjusted <= 6 ? "amber" as const : "red" as const;

  return {
    ...baseTRI,
    score: adjusted,
    band,
    formula: `${baseTRI.formula} → +${adjustment} (Physical Metal) → ${adjusted}`,
  };
}
...
/**
 * Merge physical metal blockers into the main blocker array.
 * Returns a new array — does not mutate inputs.
 */
export function mergePhysicalMetalBlockers(
  baseBlockers: PolicyBlocker[],
  pmResult: PhysicalMetalRiskResult,
): PolicyBlocker[] {
  return [...baseBlockers, ...pmResult.blockers];
}

/**
 * Merge physical metal compliance checks into the main checks array.
 * Returns a new array — does not mutate inputs.
 */
export function mergePhysicalMetalChecks(
  baseChecks: ComplianceCheck[],
  pmResult: PhysicalMetalRiskResult,
): ComplianceCheck[] {
  return [...baseChecks, ...pmResult.complianceChecks];
}
