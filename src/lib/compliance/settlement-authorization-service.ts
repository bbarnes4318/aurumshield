 ...
 *
 * CONCURRENCY: Accepts a transaction context to ensure gate writes
 * are atomic with the parent authorization write.
 */
async function persistGateResults(
  ...
  for (const gate of gateResults) {
    ...
    await tx.insert(coSettlementGates).values({
...
export async function authorizeSettlement(
  ...
  /* ════════════════════════════════════════════════════════════════
     CONCURRENCY: The entire 6-gate pipeline + authorization write
     executes inside a single serializable transaction to prevent:
       - Stale reads between gate evaluations
       - Duplicate authorizations for the same lot/buyer/rail
       - Inconsistent gate persistence
     ════════════════════════════════════════════════════════════════ */

  return await db.transaction(async (tx) => {
    const gateResults: GateResult[] = [];

    // ── IDEMPOTENCY GUARD: Check for existing AUTHORIZED auth ──
    // Prevents duplicate authorizations for the same lot/buyer/rail.
    const [existingAuth] = await tx
      .select()
      .from(coSettlementAuthorizations)
      .where(
        and(
          eq(coSettlementAuthorizations.refineryLotId, refineryLotId),
          eq(coSettlementAuthorizations.buyerSubjectId, buyerSubjectId),
          eq(coSettlementAuthorizations.paymentRail, paymentRail),
          eq(coSettlementAuthorizations.verdict, "AUTHORIZED"),
        ),
      )
      .limit(1);

    if (existingAuth) {
      // Return existing authorization — idempotent re-evaluation
      const existingGates = await tx
        .select()
        .from(coSettlementGates)
        .where(eq(coSettlementGates.settlementAuthorizationId, existingAuth.id));

      console.log(
        `[SETTLEMENT] ♻️ IDEMPOTENT: Existing authorization found for ` +
          `lot=${refineryLotId}, buyer=${buyerSubjectId}, rail=${paymentRail}. ` +
          `Returning existing auth=${existingAuth.id}`,
      );
      ...
      return {
        ...
        refineryLotId,
        buyerSubjectId,
        payableValue: existingAuth.payableValue,
        paymentRail: existingAuth.paymentRail,
        decisionHash: existingAuth.decisionHash,
        policySnapshotId: existingAuth.policySnapshotId,
        settlementAuthorizationId: existingAuth.id,
        gateResults: existingGates.map((g) => makeGateResult(
          g.gateType, 0, g.result === "PASS", g.detail ?? "",
        )),
        decidedAt: existingAuth.authorizedAt?.toISOString() ?? new Date().toISOString(),
      };
    }
    ...
    const buyerApprovalCases = await tx
      .select()
      .from(coCases)
      .where(
        and(
    ...
    const supplierSanctionsChecks = await tx
      .select()
      .from(coChecks)
    ...
    const buyerFundingChecks = await tx
      .select()
      .from(coChecks)
      .innerJoin(coCases, eq(coChecks.caseId, coCases.id))
      .where(
    ...
    const policySnapshots = await tx
      .select()
    ...
    // ════════════════════════════════════════════════════════════════
    // ALL GATES PASSED — WRITE AUTHORIZATION (within transaction)
    // ════════════════════════════════════════════════════════════════
    ...
    if (existingCases.length > 0) {
      ...
      // Close the case as APPROVED (conditional: only if still OPEN)
      await tx
        .update(coCases)
        .set({
          status: "APPROVED",
        ...
        .where(and(eq(coCases.id, complianceCaseId), eq(coCases.status, "OPEN")));
    } else {
    ...
    // ── Persist per-gate results (within same transaction) ──
    await persistGateResults(tx, authorization.id, gateResults, decisionHash);
