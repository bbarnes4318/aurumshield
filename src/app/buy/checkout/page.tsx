...
/* ================================================================
   PHASE 4: CHECKOUT — Transparent Order Review
   ================================================================
   Clear line-item breakdown: gold quote, transit, platform fee,
   optional services. Soft 60s price lock. Vendor: Column (mock)
   ================================================================ */
...
export default function CheckoutPage() {
  ...
  const { canAccess, selectedProduct, completePhase, setLockedQuote } =
    useBuyerFlow();
  const { data: priceData } = useGoldPrice();

  if (!canAccess("checkout")) {
    router.replace("/buy/marketplace");
    return null;
  }
  ...
  return (
    <CheckoutContent
      ...
      spotPrice={priceData?.spotPriceUsd ?? 2650.0}
      completePhase={completePhase}
      setLockedQuote={setLockedQuote}
      router={router}
    />
  );
}
...
function CheckoutContent({
  ...
  selectedProduct: ReturnType<typeof useBuyerFlow>["selectedProduct"];
  spotPrice: number;
  completePhase: (phase: "checkout") => void;
  setLockedQuote: ReturnType<typeof useBuyerFlow>["setLockedQuote"];
  ...
  /* ── Compute line items ── */
  const weightOz = selectedProduct?.weightOz ?? 1;
  const premiumPct =
    selectedProduct?.type === "bullion"
      ? 2.5
      : selectedProduct?.type === "nuggets"
        ? 5.0
        : 1.5;
  ...
  const handleConfirm = useCallback(() => {
    setConfirmed(true);
    setLockedQuote({
      spotPricePerOz: spotPrice,
      totalGoldCost: lineItems.goldCost,
      transitInsurance: lineItems.transit,
      platformFee: lineItems.platformFee,
      optionalServices: lineItems.optionalAssay,
      grandTotal: lineItems.grandTotal,
      lockedAt: Date.now(),
    });
    setTimeout(() => {
      completePhase("checkout");
      router.push("/buy/purchase");
    }, 600);
  }, [spotPrice, lineItems, setLockedQuote, completePhase, router]);
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="w-full max-w-lg">
        {/* ── Line Items Card ── */}
        <div className="rounded-2xl border border-slate-800 bg-[#0d1829] p-6">
          ...
          <div className="divide-y divide-slate-800/60">
            ...
            <div className="flex items-center justify-between py-4">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-slate-500">
                  {weightOz} oz × ${fmtUSD(spotPrice * (1 + premiumPct / 100))}/oz (Spot + {premiumPct}%)
                </p>
              </div>
            ...
            <div className="flex items-center justify-between py-4">
              <div>
                <p
                  className={`text-sm font-semibold ${assayEnabled ? "text-slate-300" : "text-slate-600"}`}
                >
        ...
        {/* ── Price Lock Section ── */}
        <div className="mt-6 rounded-2xl border border-slate-800 bg-[#0d1829] p-6">
          {!isLocked && !confirmed ? (
            <>
              <div className="mb-4 flex items-center justify-center gap-2">
                <Clock className="h-3.5 w-3.5 text-slate-400" />
                <p className="text-center text-xs text-slate-400">
                  Gold prices fluctuate. Lock your price for 60 seconds while
                  you confirm.
                </p>
              </div>
              <button
            ...
            <div className="space-y-4">
              {/* Locked banner */}
              <div className="flex items-center justify-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 py-4">
