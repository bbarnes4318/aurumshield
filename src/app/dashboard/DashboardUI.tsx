...
/* ================================================================
   ...
   ================================================================
   Receives DashboardData as props from the async Server Component.
   Contains all interactive UI: scenario toggle, skeleton states,
   panels, and the IntradayControlCard (with TanStack Query hooks).
   ================================================================ */
...
import { useCapitalControls, useIntradayCapital } from "@/hooks/use-mock-queries";
    ...
    <DashboardPanel title="Recent Evidence Validations" tooltip={TOOLTIPS.evidenceValidations} asOf={asOf}>
      <div className="space-y-2.5">
        {validations.map((v) => {
          ...
          return (
            <div key={v.id} className="flex items-start gap-3 rounded-(--radius-sm) bg-surface-2 px-3 py-2.5">
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-900 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] px-6 py-5 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 print:hidden">
            <Link
              href="/intraday"
              className="flex items-center gap-1.5 rounded-(--radius-input) border border-border bg-surface-2 px-3 py-1.5 text-xs font-medium text-text-muted transition-colors hover:text-text hover:bg-surface-3"
            >
...
export default function DashboardUI({ initialData, isDemo }: DashboardUIProps) {
  ...
  const d = isDemo ? getMockDashboardData(scenario) : initialData;
  const cap = d.capital;
